...
public class ALUtils {
    ...
    public class func getGooglePlaceSearchString(lan: String = "-33.8670522", lng: String = "151.1957362", radius:String = "500", types:String = "", name:String = "", keyword: String, language:String = "") -> String {
        ...
        return "\(ALFrameworkConfig.GooglePlaceAPIURLPrefix)\(lan),\(lng)\(radiusStr)\(languageStr)\(typesStr)\(nameStr)\(keywordStr)&key=\(ALFrameworkConfig.GooglePlaceServerAPIKey)"
    }
    ...
    public class func getGooglePlaceGetPlaceByIDURL(id:String, language:String = "en") -> String{
        let languageStr = language == "en" ? "" : "&language=\(language)"
         return "\(ALFrameworkConfig.GooglePlaceGetPlaceByIDURLPrefix)\(id)&key=\(ALFrameworkConfig.GooglePlaceServerAPIKey)"
    }
    public class func getGooglePlacePhotoURL(photoReference: String) -> String {
        return "\(ALFrameworkConfig.GooglePlacePhotoURLPrefix)\(photoReference)&key=\(ALFrameworkConfig.GooglePlaceServerAPIKey)"
    }
    ...
    public class func setObjectForKeyInUserDefaults(object: AnyObject, key:String, isGroupStorage: Bool = true){
        ...
        if isGroupStorage {
            defaults = NSUserDefaults(suiteName: ALFrameworkConfig.NSUserDefaultSuiteName)
        } else {
    ...
    public class func getJSONFromFile(filename:String, callback:((jsonData:AnyObject?) -> Void)){
        var error: NSError?
        let path = NSBundle.mainBundle().pathForResource(filename, ofType: "json")!
        if let data:NSData = NSData(contentsOfFile: path, options: NSDataReadingOptions.DataReadingMappedIfSafe, error: &error) {
